    ...
    internal static unsafe class HeapSortImplUnmanaged<T>
    {
        ...
        public static void Sort(T* ptr, T* ptrEnd, IComparer<T> comparer)
        {
            ...
            if (count < 2L || SortUtils.OptimizeSort(ptr, ptrEnd, count, comparer))
                return;
            SortCore(ptr, count, comparer);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void SortWithoutCheck(T* ptr, T* ptrEnd, IComparer<T> comparer)
        {
            SortCore(ptr, ptrEnd - ptr, comparer);
        }

        [Inline(InlineBehavior.Remove)]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void SortCore(T* ptr, long count, IComparer<T> comparer)
        {
            // 建立最大堆積
            long i;
            ...
            if (i > 0)
            {
                ...
                BinaryInsertionSortImplUnmanaged<T>.Sort(ptr, ptrEnd, comparer);
            }
        }
